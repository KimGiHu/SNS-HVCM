...
parser.add_argument('--name', type=str,
                    default='Baseline and Proposal', help='type the model which is plotted')
...
with torch.no_grad():
    ...
    for data_test, labels in pbar:
        ...
        latent_z_baseline_normal.append(z_normal_baseline.cpu().numpy()) # 기존 모델의 잠재공간 
        latent_z_proposed_normal.append(z_normal_proposed.cpu().numpy()) # 제안 모델의 잠재공간
...
with torch.no_grad():
    ...
    for data_anomaly,anomaly_labels in pbar:
        ...
        latent_z_baseline_abnormal.append(z_anomaly_baseline.cpu().numpy()) # 기존 모델의 잠재공간 
        latent_z_proposed_abnormal.append(z_anomaly_proposed.cpu().numpy()) # 제안 모델의 잠재공간
...
# 배열 형태로 초기화 : normal and abnormal of latent space 'z'
latent_z_baseline_normal = np.concatenate(latent_z_baseline_normal, axis=0)
latent_z_proposed_normal = np.concatenate(latent_z_proposed_normal, axis=0)

latent_z_baseline_abnormal = np.concatenate(latent_z_baseline_abnormal, axis=0)
latent_z_proposed_abnormal = np.concatenate(latent_z_proposed_abnormal, axis=0)

# 정상 및 이상신호의 latent space z간 concatenate
latent_z_normal_all = np.concatenate([latent_z_baseline_normal,latent_z_proposed_normal],axis=0)
latent_z_abnormal_all = np.concatenate([latent_z_proposed_normal,latent_z_proposed_abnormal],axis=0)
...
perplexity_normal = min(30, len(latent_z_normal_all_2d) - 1)
perplexity_abnormal = min(30, len(latent_z_abnormal_all_2d) - 1)
...
plt.scatter(latent_tsne_normal[:len(latent_z_baseline_normal), 0], latent_tsne_normal[:len(latent_z_baseline_normal), 1], label='Baseline Normal', alpha=0.6)
plt.scatter(latent_tsne_abnormal[:len(latent_z_baseline_abnormal), 0], latent_tsne_abnormal[:len(latent_z_baseline_abnormal), 1], label='Baseline Fault', alpha=0.6)
plt.title('Baseline Model t-SNE of Latent Space')
plt.xlabel('X of latent Space')
plt.ylabel('Y of latent Space')
plt.legend()
plt.grid(True)
plt.savefig('./%s/'%args.figure_path+'Basline'+' tsne_latent_space.png', dpi=600)
...
plt.figure(figsize=(10, 6))
plt.scatter(latent_tsne_normal[len(latent_z_baseline_normal):, 0], latent_tsne_normal[len(latent_z_baseline_normal):, 1], label='Proposed Normal', alpha=0.6)
plt.scatter(latent_tsne_abnormal[len(latent_z_baseline_abnormal):, 0], latent_tsne_abnormal[len(latent_z_baseline_abnormal):, 1], label='Proposed Fault', alpha=0.6)
plt.title('Proposal Model t-SNE of Latent Space')
plt.xlabel('X of latent Space')
plt.ylabel('Y of latent Space')
plt.legend()
plt.grid(True)
plt.savefig('./%s/'%args.figure_path+'Proposal'+' tsne_latent_space.png', dpi=600)
...
# 이상 탐지 시각화 : Histogram, Kernel Density Estimation, Box plot
...
# histogram settings
plt.title(f'%s Normal vs. Fault kde plot'%args.name)
...
plt.savefig('./%s/'%args.figure_path+'%s'%args.name + str(' Histogram plot') + '.png', dpi=600)
...
plt.savefig('./%s/'%args.figure_path+'%s'%args.name + str(' Kernel Density Estimation plot') + '.png', dpi=600)
...
# box plot settings
plt.title(f'%s Normal vs. Fault box plot'%args.name)
...
plt.savefig('./%s/'%args.figure_path+'%s'%args.name + str(' Box plot') + '.png', dpi=600)
