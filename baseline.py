...
# 손실함수 정의 : 베이스라인 (MSE + KLD)
def loss_function(x_recon, x, mu, logvar):    

    # MSE_lib = nn.functional.mse_loss(x_recon, x) # divided into batch size, time steps
...
if __name__ =='__main__':
    ...
    source_model.train()
    # 손실 함수 및 옵티마이저
    optimizer_source = optim.Adam(source_model.parameters(), lr=1e-5)
    ...
    torch.save(source_model.state_dict(), "./%s"%args.paths+"/baseline.pth")
